...
export const SUBMENUS = {
  mindBody: { name: 'Mind & Body', sub: 'Work on self-improvement', icon: 'meditate', items: [
    ...
    item('garden', 'Garden', 'Grow something', 8, { happiness: 2, health: 1 }, 'You tended a small garden and grew more herbs than you could use.', { icon: 'garden' }),
    item('gym', 'Gym', 'Work out at the gym', 14, { health: 4, looks: 2, happiness: 1 }, 'You kept up a steady gym routine.', { icon: 'gym', cost: 40 }),
    item('instrument', 'Instruments', 'Practice an instrument', 6, { happiness: 3, smarts: 1 }, 'You practiced your instrument until the neighbors clapped.', { icon: 'instrument', requires: 'instrument' }),
    item('library', 'Library', 'Visit the public library', 6, { smarts: 2 }, 'You spent the afternoon studying at the library.', { icon: 'library' }),
    item('martialArts', 'Martial Arts', 'Practice a martial art', 8, { health: 3, happiness: 2 }, 'You practiced martial arts and earned a new belt stripe.', { icon: 'martial', cost: 60 }),
    item('meditate', 'Meditate', 'Practice meditation', 10, { happiness: 4, health: 1 }, 'You took quiet time to meditate.', { icon: 'meditate' }),
    item('memory', 'Memory Test', 'Have your memory evaluated', 6, { smarts: 2, happiness: 1 }, 'You played memory games and beat your record.', { icon: 'memory' }),
    item('sports', 'Sports', 'Train with a team', 7, { health: 3, happiness: 2 }, 'You trained hard with your sports team.', { icon: 'ball', cost: 30 }),
    item('walk', 'Walk', 'Fresh air', 5, { health: 2, happiness: 2 }, 'You went for a long walk around the neighborhood.', { icon: 'walk' }),
  ] },
  doctor: { name: 'Doctor', sub: 'Visit the doctor', icon: 'doctor', items: [
    item('checkup', 'Checkup', 'Cedar Grove Clinic', 5, { health: 3 }, 'You went to the doctor for a checkup.', { icon: 'stethoscope', cost: 50 }),
    item('dentist', 'Dentist', 'Keep that smile', 5, { health: 1, looks: 1 }, 'You visited the dentist. No cavities!', { icon: 'scissors', cost: 80 }),
    item('eyeExam', 'Eye Exam', 'See clearly', 6, { health: 1, smarts: 1 }, 'You had your eyes checked.', { icon: 'eye', cost: 60 }),
    item('counselor', 'Counselor', 'Someone to listen', 12, { happiness: 5 }, 'You talked things through with a counselor.', { icon: 'talk', cost: 90 }),
  ...
  pets: { name: 'Pets', sub: 'Get a pet', icon: 'dog', items: Object.entries(People.PET_SPECIES).map(([species, def]) =>
  ...
  salon: { name: 'Salon & Spa', sub: 'Take time for yourself', icon: 'spa', items: [
    item('haircut', 'Hair Stylist', 'A fresh cut', 8, { looks: 2, happiness: 1 }, 'You got a fresh haircut and felt like a new person.', { icon: 'salon', cost: 30 }),
    ...
    item('massage', 'Massage', 'Unwind', 16, { health: 2, happiness: 3 }, 'You had a massage and felt the tension melt away.', { icon: 'spa', cost: 80 }),
    item('nails', 'Nail Salon', 'Get your nails done', 12, { looks: 1, happiness: 1 }, 'You got your nails done.', { icon: 'nails', cost: 40 }),
    item('spaDay', 'Spa Day', 'The full treatment', 18, { happiness: 4, health: 1, looks: 1 }, 'You spent a whole day at the spa.', { icon: 'spa', cost: 150 }),
  ] },
  licences: { name: 'Licenses', sub: 'Manage your licenses', icon: 'licence', items: [
  ...
  identity: { name: 'Identity', sub: 'Define your identity', icon: 'identity', items: [
...
export const ACTIVITY_MENU = [
  { id: 'adoption', name: 'Adoption', sub: 'Adopt a child', icon: 'adoption', action: item('adoptChild', 'Adoption', 'Adopt a child', 25, { happiness: 6 }, '', { cost: 500, special: 'adoptChild' }) },
  { id: 'clubs', name: 'Clubs', sub: 'Join a club and meet people', icon: 'clubs', action: item('joinClub', 'Join a Club', 'Meet new people', 10, { happiness: 2 }, '', { special: 'newFriend', maxAge: 40 }) },
  { id: 'doctor', name: 'Doctor', sub: 'Visit the doctor', icon: 'doctor', menu: 'doctor' },
  { id: 'emigrate', name: 'Emigrate', sub: 'Move to another country', icon: 'emigrate', action: item('emigrate', 'Emigrate', 'Move abroad', 18, { happiness: 3 }, '', { cost: 800, special: 'emigrate' }) },
  { id: 'friends', name: 'Hang Out', sub: 'Spend a weekend with friends', icon: 'hangout', action: item('hangOut', 'Hang Out with Friends', 'Needs friends', 6, { happiness: 4 }, 'You hung out with your friends all weekend.', { special: 'friends' }) },
  { id: 'identity', name: 'Identity', sub: 'Define your identity', icon: 'identity', menu: 'identity' },
  { id: 'licences', name: 'Licenses', sub: 'Manage your licenses', icon: 'licence', menu: 'licences' },
  { id: 'loan', name: 'Loan', sub: 'Borrow money', icon: 'loan', action: item('loan', 'Loan', 'Borrow $5,000', 18, {}, '', { special: 'loan' }) },
  { id: 'love', name: 'Love', sub: 'Find someone to love', icon: 'heart', menu: 'love' },
  { id: 'mindBody', name: 'Mind & Body', sub: 'Work on self-improvement', icon: 'meditate', menu: 'mindBody' },
  { id: 'movies', name: 'Movie Theater', sub: 'Go to the movies', icon: 'film', menu: 'leisure' },
  { id: 'pets', name: 'Pets', sub: 'Get a pet', icon: 'dog', screen: 'pets' },
  { id: 'playOutside', name: 'Play Outside', sub: 'Run around until dark', icon: 'playOutside', action: item('playOutside', 'Play Outside', 'Run around', 3, { happiness: 3, health: 2 }, 'You played outside until it got dark.', { maxAge: 12 }) },
  { id: 'salon', name: 'Salon & Spa', sub: 'Take time for yourself', icon: 'spa', menu: 'salon' },
  { id: 'shopping', name: 'Shopping', sub: 'Buy something', icon: 'shopping', screen: 'shopping' },
  { id: 'vacation', name: 'Vacation', sub: 'A week away', icon: 'vacation', action: item('vacation', 'Vacation', 'A week away', 18, { happiness: 8, health: 2 }, 'You took a week-long vacation.', { cost: 1200 }) },
  ...
  { id: 'volunteer', name: 'Volunteer', sub: 'Help your community', icon: 'volunteer', action: item('volunteer', 'Volunteer', 'Help your community', 13, { happiness: 3, smarts: 1 }, 'You volunteered on weekends.') },
  { id: 'will', name: 'Will & Testament', sub: 'Put your affairs in order', icon: 'will', action: item('will', 'Will & Testament', 'Write your will', 40, { happiness: 2 }, 'You wrote your will and felt a weight lift.', { cost: 200, special: 'will' }) },
  { id: 'zoo', name: 'Zoo Trip', sub: 'Meet the animals', icon: 'zoo', action: item('zoo', 'Zoo Trip', 'Meet the animals', 3, { happiness: 3, smarts: 1 }, 'You spent a day at the zoo.', { cost: 30 }) },
];
