// Visual-fidelity capture: exact 390x844 viewport at 1.5x through the DevTools protocol (matches the reference frames).
// usage: node tests/screenshot.mjs <outdir> <base-url> name=query ...   e.g. main=seed=7&age=27#main
// Needs a Chromium binary (CHROMIUM env var, or Playwright's cached build).
import { spawn } from 'node:child_process';
import { writeFileSync, mkdirSync } from 'node:fs';
const [outDir, base, ...pairs] = process.argv.slice(2);
mkdirSync(outDir, { recursive: true });
const CH = process.env.CHROMIUM || `${process.env.HOME}/Library/Caches/ms-playwright/chromium-1140/chrome-mac/Chromium.app/Contents/MacOS/Chromium`;
...
const send = (method, params = {}) => new Promise((r, j) => { const n = ++id; waiting.set(n, r); ws.send(JSON.stringify({ id: n, method, params })); setTimeout(() => { if (waiting.has(n)) { waiting.delete(n); j(new Error(method + ' timed out')); } }, 15000); });
